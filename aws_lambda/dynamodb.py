...
class DynamoDB(object):
    ...
    def table_count(self, table):
        ...
        if self.client:
            return self.dynamodb.describe_table(TableName=table)["Table"][
                "ItemCount"
            ]
        else:
    ...
    def change_capacity(self, table, read, write):
        ...
        if self.client:
            resp = self.dynamodb.update_table(
                TableName=table, ProvisionedThroughput=data
            )
        else:
            resp = self.dynamodb.Table(table).update(ProvisionedThroughput=data)
    ...
    def query(self, table, key, value, index=None, lastevaluatedkey=None):
        ...
        if resp:
            ...
            if resp["LastEvaluatedKey"]:
                n_resp, n_count = self.query(
                    table, key, value, index, resp["LastEvaluatedKey"]
                )
